...
public class Cookie {
	...
	private static final Integer STAGE_WIDTH = 750; // The pixel width of the
													// application
	private static final Integer STAGE_HEIGHT = STAGE_WIDTH; // The pixel height
																// of the
																// application

	private Image cookieImage; // The image of the cookie.
	public Circle cookieBody; // The rectangle that represents the cookie.
	private boolean isFalling; // True means the cookie is falling.
	private Random rand; // A random number generator. Used to determine where
							// cookies fall from.
	 ...
	 * 
	 * @param root
	 *            : The current group the scene is utilizing.
	 * @param level
	...
	public void populateCookie(Group root, int Level, Sprite sprite) {
		...
		cookieImage = new Image(getClass().getResourceAsStream(
				"minecraft_cookie.png"));
	...
	/**
	 * Stops the cookie from falling and brings it back to the top of the
	 * screen.
	 */
	...
	/**
	 * Determines how "intelligently" cookies fall based off of the current game
	 * level.
	 */
	...
	/**
	 * Makes some cookies fall directly on the player. Makes the rest fall
	 * randomly.
	 */
	...
	/**
	 * Makes some cookies fall directly on the player. Makes other cookies fall
	 * a little bit behind and in front of the player. Drops cookies on the
	 * edges of the screen. Makes the rest fall randomly.
	 */
	private void SmartCookiePlacement() {
		if (cookieID == 4) {
			cookieBody.setCenterX(player.getSpriteXPosition() + 3
	...
	/**
	 * Makes some of the cookies purposefully avoid the player for level 4. The
	 * rest of the cookies fall randomly.
	 */
	private void AvoidSpriteCookiePlacement() {
		if (cookieID < level * 2) {
			cookieBody.setCenterX(player.getSpriteXPosition() + 7
					* player.getSpriteWidth());
	 ...
	 * 
	 * @param ID
	 *            : The index of the cookie in the cookie array.
	 */
